---
title: "GeometryOperation"
description: ""
weight: 20
url: "/gridbuilding/v6.0/api/godot/geometryoperation/"
---

# GeometryOperation

```csharp
GridBuilding.Godot.Test.Performance
class GeometryOperation
{
    // Members...
}
```



**Project:** GridBuilding v6.0  
**Layer:** Godot  
**Source:** `Godot/Tests/Unit/Helpers/GridBuildingTestPerformance.cs`  
**Namespace:** `GridBuilding.Godot.Test.Performance`  
**Parsing Method:** AST-based (Roslyn) - NOT regex

**⚠️ IMPORTANT**: This documentation was generated using AST parsing, not regex.


## Properties

### OperationType

```csharp
public GeometryType OperationType { get; set; }
```

### ShapeA

```csharp
public ShapeData ShapeA { get; set; }
```

### ShapeB

```csharp
public ShapeData ShapeB { get; set; }
```

